...
def print_location(loc):
    ...
    print(loc[0] + " (" + loc[3] + ") at coordinates " + str(loc[1]) +  ", "+ str(loc[2]))
...
def nearest_location(loc, loc_list, loc_type=None):
    ...
    if(loc_type == None):
        for i in range(0,len(loc_list)):
            if(loc_list[i] != loc):
                if((distance(loc,loc_list[i])<d)or(d<0)):
                    d = distance(loc,loc_list[i])
                    p = i
        return loc_list[p]   
    else:
        for i in range(0,len(loc_list)):
            if((loc_list[i] != loc) and (loc_type == loc_list[i][3])):
                if((distance(loc,loc_list[i])<d)or(d<0)):
                    d = distance(loc,loc_list[i])
                    p = i
        return loc_list[p]

...
def place_blinker(board, p = (0,0)):
    ...
    if(p[0] +1 <  int(math.sqrt(len(board)))):
        set_alive(board,(p[0]+1,p[1]),True)
    if(p[0] +2<  int(math.sqrt(len(board)))):
...
def place_glider(board, p = (0,0)):
    ...
    if(p[0] +1 <  int(math.sqrt(len(board)))):
        set_alive(board,(p[0]+1,p[1]),True)
    if(p[0] +2<  int(math.sqrt(len(board))) and (p[1] +1<  int(math.sqrt(len(board))))):
        set_alive(board,(p[0]+2,p[1]+1),True)
    if(p[0] +0<  int(math.sqrt(len(board))) and (p[1] +2<  int(math.sqrt(len(board))))):
        set_alive(board,(p[0]+0,p[1]+2),True)
    if(p[0] +1<  int(math.sqrt(len(board))) and (p[1] +2<  int(math.sqrt(len(board))))):
        set_alive(board,(p[0]+1,p[1]+2),True)
    if(p[0] +2<  int(math.sqrt(len(board))) and (p[1] +2<  int(math.sqrt(len(board))))):
...
def tick(board):
    ...
    """
    """GRAPSTE TON KWDIKA SAS EDW."""
    game2 = copy_board(board)
    for i in range (int(math.sqrt(len(board)))):
        for j in range (int(math.sqrt(len(board)))):
            count = 0
            l = neighbors((i,j))
            for k in l:
                if(k[0] >= 0 and k[1] >=0 and k[0]<int(math.sqrt(len(board))) and k[1]<int(math.sqrt(len(board)))):
                    if(is_alive(game2,k)):
                        count = count + 1
            if(is_alive(game2,(i,j)) == True and count < 2):
                set_alive(board,(i,j),False)
            elif(is_alive(game2,(i,j)) == True and count > 3):
                set_alive(board,(i,j),False)
            elif(is_alive(game2,(i,j)) == False and count == 3):
